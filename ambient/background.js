// ambient/background.js — Xiaomi MiMo style fluid WebGL background with hope palette colors
...
const FRAG = /* glsl */`
  ...
  void main() {
    ...
    float t = uTime * 0.10; // slow master time

    // Pointer bias: pull the background forms toward cursor
    vec2 ptrOff = uPointer * 0.35 * uPointerEased;
    vec2 pp = p - ptrOff;

    // 4 large drifting metaballs in lissajous paths
    ...
    // Dynamic color shifting through hope pastel color ranges
    float a = 0.5 + 0.5 * sin(uHueShift * 6.28318);
    float b = 0.5 + 0.5 * sin(uHueShift * 6.28318 + 2.094);
    
    // Shading using hues from our hope palette (peach: 16, lavender: 265, mint: 144, sky: 208, rose: 354)
    float h1 = mix(354.0/360.0, 265.0/360.0, a);  // rose ↔ lavender
    float h2 = mix(265.0/360.0, 208.0/360.0, b);  // lavender ↔ sky
    float h3 = mix(208.0/360.0, 144.0/360.0, 1.0 - b); // sky ↔ mint

    vec3 cA = hsl2rgb(vec3(h1, 0.70, 0.85)); // soft capped pastels
    vec3 cB = hsl2rgb(vec3(h2, 0.60, 0.88));
    vec3 cC = hsl2rgb(vec3(h3, 0.55, 0.86));
    ...
    // Add soft fold highlights
    float fold = smoothstep(0.4, 0.95, abs(disp) * 2.0);
    col += hsl2rgb(vec3(h2, 0.5, 0.92)) * fold * 0.25;

    // Deep void base: dark calming plum (#0d081b)
    vec3 deep = vec3(0.05, 0.03, 0.11);
    col = mix(deep, col, 0.65 + 0.35 * fbm(p * 2.0 + t * 0.5));

    // Vignette
    float vig = smoothstep(1.5, 0.2, length(vUv - 0.5) * 1.4);
    col *= mix(0.4, 1.0, vig);

    // Slow breath pulsing (6s period)
    col *= 0.95 + 0.05 * sin(uTime * 1.047);
...
export function mountBackground(canvas) {
  ...
  function frame() {
    ...
    uniforms.uHueShift.value = (t * speed * 0.02) % 1.0;
    ...
    uniforms.uPointerEased.value = pointerEased * 1.2;
